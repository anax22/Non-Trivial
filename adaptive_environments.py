# the code for the prototype is the same as linear_prototype, the change is adding an adaptive environment instead of a static one
...
# added code
    def select_action(self, action_space, level):
        if random.uniform(0, 1) < self.rvalue:
            return random.choice(action_space)
        else:
            values = getattr(self, f"{level}")
            return max(action_space, key=lambda action: values[action])
       
    def take_high_level_action(self, action, adjustment=0.05):
        self.high_level_goals[action] += adjustment

    def take_mid_level_action(self, action, adjustment=0.05):
        self.mid_level_goals[action] += adjustment

    def take_low_level_action(self, action, adjustment=0.05):
        self.low_level_goals[action] += adjustment
...
# end of added code
    ...
    def step(self):
        ...
        high_level_action = self.select_action(["Safety", "Efficiency"], level="high_level_goals")
        mid_level_action = self.select_action(["TrafficManagement", "RouteOptimization"], level="mid_level_goals")
        low_level_action = self.select_action(["Acceleration", "Braking"], level="low_level_goals")
        self.take_high_level_action(high_level_action)
        self.take_mid_level_action(mid_level_action)
        self.take_low_level_action(low_level_action)
